...
describe('resend with existing key', () => {
    ...
    describe('initial key available', () => {
        beforeEach(async () => {
            await getLocalGroupKeyStore(await subscriber.getUserId()).set(initialKey.id, hexToBinary(publisherWallet.address), initialKey.data)
        })
    ...
    describe('rotated key available', () => {
        beforeEach(async () => {
            await getLocalGroupKeyStore(await subscriber.getUserId()).set(rotatedKey.id, hexToBinary(publisherWallet.address), rotatedKey.data)
        })
    ...
    describe('rekeyed key available', () => {
        beforeEach(async () => {
            await getLocalGroupKeyStore(await subscriber.getUserId()).set(rekeyedKey.id, hexToBinary(publisherWallet.address), rekeyedKey.data)
        })
        it('can\'t decrypt initial', async () => {
            await assertNonDecryptable(1000, 2000)
        })
