...
import { collect, toUserId, toUserIdRaw, wait } from '@streamr/utils'
...
describe('parallel key exchange', () => {
    ...
    beforeAll(async () => {
        ...
        await Promise.all(PUBLISHERS.map(async (publisher) => {
            ...
            await publisher.client.addEncryptionKey(publisher.groupKey, toUserIdRaw(toUserId(publisher.wallet.address)))
        }))
    }, 20000)
