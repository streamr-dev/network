...
import { getInternalResendOptions, ResendOptions, Resends } from './Resends'
...
export const initResendSubscription = (
    ...
    const resendThenRealtime = async function* (src: AsyncGenerator<StreamMessage>): AsyncGenerator<StreamMessage, void, any> {
        try {
            const resentMsgs = await resends.resend(subscription.streamPartId, getInternalResendOptions(resendOptions), getStorageNodes)
