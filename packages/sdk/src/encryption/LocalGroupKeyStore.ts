import { Logger, StreamID, UserIDOld } from '@streamr/utils'
...
function formLookupKey2(publisherId: UserIDOld, streamId: StreamID): string {
...
export class LocalGroupKeyStore {
    ...
    async get(keyId: string, publisherId: UserIDOld): Promise<GroupKey | undefined> {
    ...
    async set(keyId: string, publisherId: UserIDOld, data: Buffer): Promise<void> {
        const persistence = await this.persistenceManager.getPersistence(NAMESPACES.ENCRYPTION_KEYS)
    ...
    async setLatestEncryptionKeyId(keyId: string, publisherId: UserIDOld, streamId: StreamID): Promise<void> {
    ...
    async getLatestEncryptionKeyId(publisherId: UserIDOld, streamId: StreamID): Promise<string | undefined> {
        const persistence = await this.persistenceManager.getPersistence(NAMESPACES.LATEST_ENCRYPTION_KEY_IDS)
