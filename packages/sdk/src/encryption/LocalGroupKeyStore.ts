...
function formLookupKey1(keyId: string, publisherId: Uint8Array): string {
    return `${binaryToHex(publisherId, true)}::${keyId}`
}

function formLookupKey2(publisherId: Uint8Array, streamId: StreamID): string {
...
export class LocalGroupKeyStore {
    ...
    async get(keyId: string, publisherId: Uint8Array): Promise<GroupKey | undefined> {
    ...
    async set(keyId: string, publisherId: Uint8Array, data: Buffer): Promise<void> {
        const persistence = await this.persistenceManager.getPersistence(NAMESPACES.ENCRYPTION_KEYS)
    ...
    async setLatestEncryptionKeyId(keyId: string, publisherId: Uint8Array, streamId: StreamID): Promise<void> {
    ...
    async getLatestEncryptionKeyId(publisherId: Uint8Array, streamId: StreamID): Promise<string | undefined> {
        const persistence = await this.persistenceManager.getPersistence(NAMESPACES.LATEST_ENCRYPTION_KEY_IDS)
