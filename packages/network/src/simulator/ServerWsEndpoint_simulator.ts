...
function cleanSocket(httpServer: http.Server | https.Server, config: UnixSocket) {
    httpServer.on('error', (err: any) => {
        ...
        staticLogger.info(`socket in use, trying to recover: ${config}`)
        ...
        clientSocket.on('error', (err: any) => {
            ...
            try {
                staticLogger.trace(`cleaning unused socket: ${config}`)
        ...
        clientSocket.once('connect', () => {
            ...
            staticLogger.error(`Another server already running on socket: ${config}`)
            process.exit(1)
        })
