    ...
    describe('after sending storage node query to tracker', () => {
        ...
        test('if tracker responds with storage nodes, connects to them one by one until success', (done) => {
            ...
            setImmediate(() => {
                ...
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-1', 'tracker', false)
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-2', 'tracker', false)
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-3', 'tracker', false)
                done()
            })
        })
