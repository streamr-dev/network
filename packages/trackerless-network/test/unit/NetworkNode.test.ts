...
import { Events } from '../../src/logic/DeliveryLayer'
...
describe('NetworkNode', () => {
    ...
    it('message listener', async () => {
        const deliveryLayer = new EventEmitter<Events>()
        const stack: Partial<NetworkStack> = {
            getDeliveryLayer: () => deliveryLayer as any,
        ...
        deliveryLayer.emit('newMessage', msg1)
        deliveryLayer.emit('newMessage', msg2)
        ...
        deliveryLayer.emit('newMessage', createMessage(3))
        expect(onMessage).toBeCalledTimes(2)
    })
})
