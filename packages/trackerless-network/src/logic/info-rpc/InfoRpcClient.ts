...
export class InfoRcClient {
    ...
    async getInfo(node: PeerDescriptor, getConnectionManagerInfo: boolean, getLayer0DhtNodeInfo: boolean, streamParts: StreamPartID[]): Promise<InfoResponse> {
        const remote = new RemoteInfoRpcServer(node, INFO_RPC_SERVICE_ID, this.infoRpcClient)
        return remote.getInfo(node, getConnectionManagerInfo, getLayer0DhtNodeInfo, streamParts)
    }
}