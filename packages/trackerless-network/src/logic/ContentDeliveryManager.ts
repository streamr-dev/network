...
import { SPLIT_AVOIDANCE_LIMIT, StreamPartSplitAvoidance } from './StreamPartSplitAvoidance'
...
export class ContentDeliveryManager extends EventEmitter<Events> {
    ...
    joinStreamPart(streamPartId: StreamPartID): void {
        ...
        layer1Node.on('manualRejoinRequired', async () => {
            if (!streamPartReconnect.isRunning() && !splitAvoidance.isNetworkSplitAvoidanceRunning()) {
    ...
    private async startLayersAndJoinDht(streamPartId: StreamPartID, entryPointDiscovery: EntryPointDiscovery): Promise<void> {
        ...
        if (this.knownStreamPartEntryPoints.has(streamPartId)) {
            ...
            const entryPoints = await entryPointDiscovery.discoverEntryPointsFromDht()
            await Promise.all([
                streamPart.layer1Node.joinDht(sampleSize(entryPoints, SPLIT_AVOIDANCE_LIMIT)),
                streamPart.layer1Node.joinRing()
            ])
            if (entryPoints.length < ENTRYPOINT_STORE_LIMIT) {
                await entryPointDiscovery.storeAndKeepSelfAsEntryPoint()
                if (streamPart.layer1Node.getNeighborCount() < SPLIT_AVOIDANCE_LIMIT) {
