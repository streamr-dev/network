...
const run = async () => {
    ...
    serverCommunicator1.on('OUTGOING_MESSAGE', (msgBody: Uint8Array, callContext?: ProtoCallContext) => {
    ...
    serverCommunicator2.on('OUTGOING_MESSAGE', (msgBody: Uint8Array, callContext?: ProtoCallContext) => {
        if (callContext!.sourceId) {
            const clientId = callContext!["sourceId"] as string
            clientCommunicators[clientId].handleIncomingMessage(msgBody)
        }
    ...
    communicator1.on('OUTGOING_MESSAGE', (msgBody: Uint8Array, clientContext?: ProtoCallContext) => {
    ...
    communicator2.on('OUTGOING_MESSAGE', (msgBody: Uint8Array, clientContext?: ProtoCallContext) => {
        let server: RpcCommunicator
