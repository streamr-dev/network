...
const runTest = async (latencyType: LatencyType) => {
    ...
    nodes.forEach((node) => {
        expect(node.getPeerCount()).toBeGreaterThanOrEqual(NUM_OF_NODES_PER_KBUCKET / 2)
        expect(node.getClosestContacts().length).toBeGreaterThanOrEqual(NUM_OF_NODES_PER_KBUCKET / 2)
    })
    expect(entryPoint.getPeerCount()).toBeGreaterThanOrEqual(NUM_OF_NODES_PER_KBUCKET / 2)
