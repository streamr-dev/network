...
export class WebrtcConnectorRpcLocal implements IWebrtcConnectorRpc {
    ...
    connect(targetPeerDescriptor: PeerDescriptor): ManagedConnection {
        ...
        if (offering) {
            managedConnection = new ManagedWebrtcConnection(this.localPeerDescriptor!, connection, undefined, targetPeerDescriptor)
        } else {
            managedConnection = new ManagedWebrtcConnection(this.localPeerDescriptor!, undefined, connection, targetPeerDescriptor)
        }
    ...
    private onRtcOffer(
        ...
        if (this.stopped || !areEqualPeerDescriptors(targetPeer, this.localPeerDescriptor!)) {
            return
        }
    ...
    private onRtcAnswer(
        ...
        if (this.stopped || !areEqualPeerDescriptors(targetPeerDescriptor, this.localPeerDescriptor!)) {
            return
        }
