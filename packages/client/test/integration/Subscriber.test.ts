...
import { getPublishTestMessages, createTestStream, getCreateClient, describeRepeats, collect, createRelativeTestStreamId } from '../test-utils/utils'
...
describeRepeats('Subscriber', () => {
    ...
    beforeEach(async () => {
        ...
        partition++
        client.debug('connecting before test <<')
        publishTestMessages = getPublishTestMessages(client, getStreamDefinition())
    })

    afterEach(async () => {
        client.debug('after test')
        expect(await M.count()).toBe(0)
        expect(await M.count(getStreamDefinition())).toBe(0)
    ...
    describe('basics', () => {
        it('works when passing stream', async () => {
            const sub = await M.subscribe(getStreamDefinition())
            expect(await M.count(getStreamDefinition())).toBe(1)
        ...
        // await expect(() => (
        // M.subscribe(getStreamDefinition())
        ...
        describe('subscription error handling', () => {
            it('works when error thrown inline', async () => {
                ...
                const sub = (await M.subscribe(getStreamDefinition())).pipe(async function* ThrowError(s) {
                    let count = 0
            ...
            it('keeps other subscriptions running if one subscription errors', async () => {
                const err = new Error('expected')
                const sub1 = await M.subscribe(getStreamDefinition())
                const sub2 = await M.subscribe(getStreamDefinition())
            ...
            it('errors subscription onMessage callback do trigger onError', async () => {
                ...
                let count = 0
                const received1: any[] = []
                const sub1 = await M.subscribe(getStreamDefinition(), (content) => {
                    if (count === MAX_ITEMS) {
                        sub1.debug('throwing...')
                        throw err
                    }
            ...
            it('multiple publishers: keeps other subscriptions running if pipeline has error but subscription onError ignores it', async () => {
                ...
                const publishTestMessages2 = getPublishTestMessages(client2, getStreamDefinition())
                const publishTask = publishTestMessages(NUM_MESSAGES, {
                    timestamp: 111111,
                })
                publishTask.catch(() => {})
    ...
    describe('ending a subscription', () => {
        ...
        it('can subscribe to stream multiple times, get updates then unsubscribe', async () => {
            ...
            expect(await M.count(getStreamDefinition())).toBe(2)
        ...
        it('can subscribe then unsubscribe in parallel', async () => {
            const [sub] = await Promise.all([
                M.subscribe(getStreamDefinition()),
                M.unsubscribe(getStreamDefinition()),
            ])
    ...
    describe('mid-stream stop methods', () => {
        ...
        beforeEach(async () => {
            sub1 = await M.subscribe(getStreamDefinition())
            sub2 = await M.subscribe(getStreamDefinition())
        ...
        it('can subscribe to stream multiple times then unsubscribe all mid-stream', async () => {
            ...
            const [received1, received2] = await Promise.all([
                ...
                collect(sub2, async ({ received }) => {
                    ...
                    if (received.length === MAX_ITEMS) {
                        await M.unsubscribe(getStreamDefinition())
