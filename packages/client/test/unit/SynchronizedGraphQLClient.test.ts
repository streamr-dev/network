...
describe('SynchronizedGraphQLClient', () => {
    ...
    it('timeout', async () => {
        ...
        theGraphIndex.start()
        return expect(() => client.sendQuery(MOCK_QUERY)).rejects.toThrow('The Graph did not synchronize to block 999999 (timed out in 1000 ms)')
    })
    ...
    it('one query timeouts, another succeeds', async () => {
        ...
        const responsePromise2 = client.sendQuery(MOCK_QUERY)
        theGraphIndex.start()
        await expect(() => responsePromise1).rejects.toThrow('The Graph did not synchronize to block 7 (timed out in 1000 ms)')
