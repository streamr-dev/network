...
describe('EncryptionUtil', () => {
    it('aes decryption after encryption equals the initial plaintext', () => {
        ...
        const ciphertext = EncryptionUtil.encryptWithAES(Buffer.from(plaintext, 'utf8'), key)
        expect(EncryptionUtil.decryptWithAES(ciphertext, key).toString('utf8')).toStrictEqual(plaintext)
    })
    ...
    it('aes encryption preserves size (plus iv)', () => {
        ...
        const ciphertext = EncryptionUtil.encryptWithAES(plaintextBuffer, key)
    ...
    it('multiple same encrypt() calls use different ivs and produce different ciphertexts', () => {
        const key = GroupKey.generate()
        const plaintext = 'some random text'
        const ciphertext1 = EncryptionUtil.encryptWithAES(Buffer.from(plaintext, 'utf8'), key)
        const ciphertext2 = EncryptionUtil.encryptWithAES(Buffer.from(plaintext, 'utf8'), key)
