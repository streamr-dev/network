...
const main = async () => {
    ...
    setInterval(async () => {
        ...
        const stream = StreamPartIDUtils.parse('streams.dimo.eth/firehose/weather#0')
        console.log('total stream neighbors on streams.dimo.eth/firehose/weather: ' + node.getNeighbors(stream).length)
    }, 10000)
