...
import { OperatorValueBreachWatcherHelper } from './OperatorValueBreachWatcherHelper'
...
export class OperatorValueBreachWatcher {
    ...
    private readonly helper: OperatorValueBreachWatcherHelper
    private readonly abortController: AbortController

    constructor(config: OperatorServiceConfig) {
        this.helper = new OperatorValueBreachWatcherHelper(config)
    ...
    private async watchOperators(): Promise<void> {
        ...
        const { sponsorshipAddresses, approxValues, realValues } = await this.helper.getApproximatePoolValuesPerSponsorship(operatorId)
