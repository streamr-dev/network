    ...
    describe('after sending storage node query to tracker', () => {
        ...
        test('if tracker responds with storage nodes, connects to them one by one until success', (done) => {
            ...
            setImmediate(() => {
                ...
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-1', 'tracker', undefined, false, true)
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-2', 'tracker', undefined, false, true)
                expect(nodeToNode.connectToNode).toBeCalledWith('storageNode-3', 'tracker', undefined, false, true)
                done()
            })
        })
