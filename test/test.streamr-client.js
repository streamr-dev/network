...
describe('StreamrClient', function() {
	...
	function createSocketMock() {
		...
		s.subscribeHandler = function(request) {
			async(function() {
				s.fakeReceive([0, 2, null, {stream: request.stream, partition: 0}])
			})
		}

		s.unsubscribeHandler = function(request) {
			async(function() {
				s.fakeReceive([0, 3, null, {stream: request.stream, partition: 0}])
			})
		}
	...
	describe("subscribe", function() {
		...
		it('should add any subscription options to subscription request', function(done) {
			...
			client.connect()
			client.connection.once('connected', function() {
				client.subscribe("stream1", 'auth', function(message) {}, {foo: 'bar'})
			})
		})
	...
	describe("subscribe with resend options", function() {
		...
		it('should emit a resend request after subscribed', function(done) {
			const sub = client.subscribe("stream1", "auth", function(message) {}, {resend_all:true})
		...
		it('should emit a resend request with given other options', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, {resend_all:true, foo: 'bar'})
			client.connect()
		})

		it('should throw an error if multiple resend options are given', function() {
			assert.throws(function() {
				client.subscribe("stream1", "auth", function(message) {}, {resend_all:true, resend_last:5})
			})
		})
		...
		it('should resend to multiple subscriptions as per each resend option', function(done) {
			socket.resendHandler = function(request) {
				...
				if (request.resend_all) {
					async(function() {
						socket.fakeReceive([0, resendingCode, request.sub, {stream:'stream1', partition: 0}])
						...
						socket.fakeReceive([0, resentCode, request.sub, {stream:'stream1', partition: 0}])
					})
				}
		...
		it('should not crash on resent if bye message is received while resending', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, {resend_all:true})
			client.connect()
		})
		...
		it('should not crash if messages exist after the bye message', function(done) {
			socket.resendHandler = function(request) {
				async(function() {
					...
					socket.fakeReceive([0, resentCode, request.sub, {stream:'stream1', sub:sub.id}])
					done()
				})
			}

			var sub = client.subscribe("stream1", "auth", function(message) {}, {resend_all:true})
			client.connect()
		})
	})
	...
	describe("message handling", function() {
		...
		it('should call the callback when a message is received', function(done) {
			client.subscribe("stream1", "auth", function() {
				done()
			})
			client.connect()
			client.connection.once('subscribed', function() {
		...
		it('should emit unsubscribe after processing a message with the bye key', function(done) {
			var processed = false
			client.subscribe("stream1", "auth", function(message) { processed = true })
			client.connect()
	...
	describe('unsubscribe', function() {
		...
		it('should handle messages after resubscribing', function(done) {
			...
			sub.on('unsubscribed', function() {
				var newSub = client.subscribe("stream1", "auth", function(message) {
					assert.deepEqual(message, { count: 0})
					done()
				})
		...
		it('should disconnect if all subscriptions are done during resend', function(done) {
			...
			var sub1 = client.subscribe("stream1", "auth", function(message) {}, {resend_all: true})
			client.connect()
			...
			client.connection.on('resending', function(request) {
				async(function() {
					...
					socket.fakeReceive([0, resentCode, null, {stream: request.stream, partition: 0, sub: request.sub }])
				})
			})
	...
	describe("pause", function() {
		...
		it('should subscribe to both old and new subscriptions after pause-and-connect', function(done) {
			...
			sub1 = client.subscribe("stream1", "auth", function(message) {})
			client.connect()
	...
	describe("resend", function() {
		...
		beforeEach(function() {
			...
			function resend(stream, sub, from, to) {
				...
				socket.fakeReceive([0, resendingCode, null, { stream: stream, sub: sub }])
				for (var i=from;i<=to;i++) {
					socket.fakeReceive([0, unicastCode, sub, msg(stream, i, {}, sub)])
				}
				socket.fakeReceive([0, resentCode, null, { stream: stream, sub: sub }])
			}
				...
				async(function() {
					...
					if (request.resend_all) {
						if (resendLimits[request.stream] === undefined) {
							...
							socket.fakeReceive([0, noResendCode, null, { stream: request.stream, sub: request.sub }])
						} else {
		...
		it('should recognize the resend_from option', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, { resend_from: 7 })
			client.connect()
		...
		it('should recognize the resend_last option', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, { resend_last: 3 })
			client.connect()
		...
		it('should throw if resend_from_time is in invalid format', function() {
			assert.throws(function() {
				client.subscribe("stream1", "auth", function(message) {}, { resend_from_time: "invalid" })
			})
		})
		...
		it('should include any subscription options in resend request', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, { auth: 'foo' })
			client.connect()
		...
		it('should not include stronger resend requests in gap resend request', function(done) {
			...
			client.subscribe("stream1", "auth", function(message) {}, { auth: 'foo', resend_all: true })
			client.connect()
		...
		describe('on reconnect', function() {
			...
			it('no resend', function(done) {
				client.subscribe("stream", "auth", msgHandler)
				client.connect()
			...
			it('resend_all', function(done) {
				...
				client.subscribe("stream", "auth", msgHandler, { resend_all: true })
				client.connect()
			...
			it('resend_from', function(done) {
				...
				client.subscribe("stream", "auth", msgHandler, { resend_from: 3 })
				client.connect()
			...
			it('resend_last', function(done) {
				...
				client.subscribe("stream", "auth", msgHandler, { resend_last: 1 })
				client.connect()
			...
			it('resend_last should accept a gap on reconnect', function(done) {
				...
				client.connection.once('disconnected', function() {
					socket.resendHandler = function (request) {
						...
						socket.fakeReceive([0, resendingCode, null, { stream: request.stream, sub: request.sub }])
						socket.fakeReceive([0, unicastCode, request.sub, msg(request.stream, 10, {}, request.sub, 9)])
						socket.fakeReceive([0, resentCode, null, { stream: request.stream, sub: request.sub }])
