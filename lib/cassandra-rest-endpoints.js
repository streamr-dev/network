...
module.exports = function(app, cassandra) {
	app.get('/api/v1/streams/:id/data/last/:count?', function(req, res) {
		var dataPoints = []

		cassandra.getLast(req.params.id, parseInt(req.params.count) || 1, function(dataPoint) {
	...
	app.get('/api/v1/streams/:id/data/fromOffset/:offset', function(req, res) {
		var dataPoints = []
		...
		if (fromOffset === Number.NaN) {
			...
			cassandra.getFromOffset(req.params.id, parseInt(req.params.offset) || Number.MAX_SAFE_INTEGER, function (dataPoint) {
					dataPoints.push(dataPoint)
				},
				function (err) {
					if (err) {
	...
	app.get('/api/v1/streams/:id/data/offsetRange/:fromOffset/to/:toOffset', function(req, res) {
		var dataPoints = []
		...
		if (fromOffset === Number.NaN) {
			...
			cassandra.getOffsetRange(req.params.id, fromOffset, toOffset, function (dataPoint) {
					dataPoints.push(dataPoint)
				},
				function (err) {
					if (err) {
	...
	app.get('/api/v1/streams/:id/data/fromTimestamp/:timestamp', function(req, res) {
		var dataPoints = []
		...
		if (timestamp === Number.NaN) {
			...
			cassandra.getFromTimestamp(req.params.id, new Date(timestamp), function (dataPoint) {
					dataPoints.push(dataPoint)
				},
				function (err) {
					if (err) {
	...
	app.get('/api/v1/streams/:id/data/timestampRange/:fromTimestamp/to/:toTimestamp', function(req, res) {
		var dataPoints = []
		...
		if (fromTimestamp === Number.NaN) {
			...
			cassandra.getTimestampRange(req.params.id, new Date(fromTimestamp), new Date(toTimestamp), function (dataPoint) {
					dataPoints.push(dataPoint)
				},
				function (err) {
					if (err) {
