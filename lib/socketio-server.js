...
SocketIoServer.prototype.handleResendRequest = function(socket, req) {
	...
	var resendProtocolWrapper = function(first, last, resendFunc) {
		...
		debugProtocol('resending: %s: %o', socket.id, resendingMsg)
		socket.emit('resending', resendingMsg)
		resendFunc(function() {
			var resentMsg = {channel: req.channel, sub: req.sub, from:first, to:last}
			debugProtocol('resent: %s: %o', socket.id, resentMsg)
			socket.emit('resent', resentMsg)			
		})
	}
	var tryStartResend = function(from, to) {
		if (from!=null && to!=null) {
			if (to<0 || to<from) {
				...
				debugProtocol('no_resend: %s: %o', socket.id, noResendMsg)
				socket.emit('no_resend', noResendMsg)
			}
			else {
...
SocketIoServer.prototype.handleSubscribeRequest = function(socket, request) {
	...
	if (!request.channel) {
		...
		debugProtocol('subscribed (error): %s: %o', socket.id, response)
		...
		var onSubscribe = function(counter) {
			...
			socket.join(stream.id, function(err) {
				if (err) {
					...
					debug("Socket %s is now in rooms: %o", socket.id, socket.rooms)
					var subscribedMsg = {
						channel: stream.id,
						from: counter
					}
					debugProtocol('subscribed: %s: %o', socket.id, subscribedMsg)
					socket.emit('subscribed', subscribedMsg)
				}
			})
		}
...
SocketIoServer.prototype.checkRoomEmpty = function(streamId) {
	...
	if (room && Object.keys(room).length>0) {
		debug("checkRoomEmpty: Clients remaining on stream %s: %i", streamId, Object.keys(room).length)
	}
	else {
...
SocketIoServer.prototype.handleUnsubscribeRequest = function(socket, request) {
	...
	debug("handleUnsubscribeRequest: socket %s unsubscribed from stream %s", socket.id, request.channel)

	socket.leave(request.channel, function(err) {
		if (err)
			console.log("ERROR leaving stream "+request.channel+": "+err)

		debug("handleUnsubscribeRequest: Socket %s is now in rooms: %o", socket.id, socket.rooms)
...
SocketIoServer.prototype.handleDisconnectRequest = function(socket) {
	var _this = this
	debug("handleDisconnectRequest: socket %s was on streams: %o", socket.id, socket.rooms)
