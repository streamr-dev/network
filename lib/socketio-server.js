...
function SocketIoServer(http, realtimeAdapter, historicalAdapter, wss) {
	...
	this.realtimeAdapter.on('message', function(messageWithKafkaMetadata) {
		_this.broadcastMessage(messageWithKafkaMetadata)
	})
...
SocketIoServer.prototype.broadcastMessage = function(messageWithKafkaMetadata) {
	var msg = messageWithKafkaMetadata.getMessage()
	var buf = protocol.encodeForBrowser(protocol.BROWSER_MSG_TYPE_BROADCAST, messageWithKafkaMetadata)

	var connections = this.getStreamObject(msg.streamId, msg.streamPartition).getConnections()
