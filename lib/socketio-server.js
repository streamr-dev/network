...
function SocketIoServer(http, realtimeAdapter, historicalAdapter, latestOffsetFetcher, wss, authenticator) {
	var _this = this
...
SocketIoServer.prototype.handleResendRequest = function(connection, req) {
	...
	this.authenticator.authenticate(streamId, authkey, 'read')
		.then(this.latestOffsetFetcher.fetchOffset(streamId, streamPartition))
		.then(function(latestKnownOffset) {
			// Resend all
...
SocketIoServer.prototype.handleSubscribeRequest = function(connection, request) {
	...
	if (!request.channel) {
		...
		this.authenticator.authenticate(streamId, authKey, 'read').then(function() {
