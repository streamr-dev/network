...
function SocketIoServer(http, realtimeAdapter, historicalAdapter, wss) {
	var _this = this
	...
	this.realtimeAdapter.on('message', function(message, streamId, streamPartition) {
		_this.broadcastMessage(message, streamId, streamPartition)
	})

	this.wss = wss || new WebSocketServer({ server: http });
...
SocketIoServer.prototype.handleResendRequest = function(connection, req) {
	...
	if (req.resend_all===true) {
		this.historicalAdapter.getAll(streamId, streamPartition, msgHandler, doneHandler)
	}
	// Resend range
	else if (req.resend_from != null && req.resend_to != null) {
		this.historicalAdapter.getOffsetRange(streamId, streamPartition, req.resend_from, req.resend_to, msgHandler, doneHandler)
	}
	// Resend from a given offset 
	else if (req.resend_from != null) {
		this.historicalAdapter.getFromOffset(streamId, streamPartition, req.resend_from, msgHandler, doneHandler)
	}
	...
	// Resend the last N messages
		...
		this.historicalAdapter.getLast(streamId, streamPartition, req.resend_last, msgHandler, doneHandler)
	}
	else {
...
SocketIoServer.prototype.broadcastMessage = function(message, streamId, streamPartition) {
	var buf = protocol.encodeForBrowser(protocol.BROWSER_MSG_TYPE_BROADCAST, message)
	var connections = this.getStreamObject(streamId, streamPartition).getConnections()

	for (var i=0; i<connections.length; i++) {
		connections[i].sendRaw(buf)
	}
}
...
SocketIoServer.prototype.handleUnsubscribeRequest = function(connection, request, noAck) {
	...
	streams[key].removeConnection(connection)
	...
	if (!noAck) {
		connection.sendUnsubscribed({channel: streamId, partition: streamPartition, error: err})
	}
}
