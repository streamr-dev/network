...
/*static*/ StreamrBinaryMessageWithKafkaMetadata.fromBytes = function(buf) {
	// Redis suffixes some fields to the buffer
	var reader = new BufferReader(buf)
	var streamrBinaryMessage = StreamrBinaryMessage.fromBytes(reader)
...
StreamrBinaryMessageWithKafkaMetadata.prototype.toArray = function(contentAsBuffer = true) {
	...
	return [
		...
		this.previousOffset,
		m.contentType,
		contentAsBuffer ? m.getContentBuffer().toString('utf8') : m.getContentParsed()
	]
}
...
StreamrBinaryMessageWithKafkaMetadata.prototype.toObject = function(contentAsBuffer = true) {
	...
	return {
		...
		content: contentAsBuffer ? m.getContentBuffer().toString('utf8') : m.getContentParsed()
	}
}
