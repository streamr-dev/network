...
KafkaHelper.prototype.unsubscribe = function(topic, cb) {
	...
	this.consumer.removeTopics([topic], function (err, removed) {
		if (err)
			...
			console.log("Unsubscribed Kafka from topic: "+topic)
...
KafkaHelper.prototype.decodeMessage = function(message, includeTimestamp) {
	...
	decodeResult.message._S = message.topic
	decodeResult.message._C = message.offset
	if (includeTimestamp)
		decodeResult.message._T = decodeResult.timestamp
...
KafkaHelper.prototype.getFirstOffsetAfter = function(topic, partition, date, cb) {
	...
	var client = this.createClient()
	var consumer = this.createConsumer(client)
	...
	client.on('ready', function() {
		...
		_this.getOffset(topic, true, function(firstOffset) {
			_this.getOffset(topic, false, function(lastOffset) {
				console.log("firstOffset: "+firstOffset+", lastOffset: "+lastOffset);
				_this.getTimestampForOffset(client, consumer, topic, partition, firstOffset, function(firstDate, firstDateError) {
					if (firstDateError) {
						close()
						cb(undefined, firstDateError)
					}
					else _this.getTimestampForOffset(client, consumer, topic, partition, lastOffset-1, function(lastDate, lastDateError) {
						if (lastDateError) {
							close()
						    ...
						    else _this.binarySearchOffsetForDate(client, consumer, topic, partition, date, firstOffset, lastOffset-1, function(offset, searchError) {
						    	close()
						    	if (searchError)
...
KafkaHelper.prototype.getTimestampForOffset = function(client, consumer, topic, partition, offset, cb) {
	var _this = this

	consumer = new EventEmitter()
	...
	var messageHandler = function(message) {
		...
		console.log("getTimestampForOffset: Response for topic: "+topic+", partition: "+partition+", offset: "+offset+", timestamp: "+decoded._T)
		unbindAll()
		cb(decoded._T)
	}
	...
	console.log("getTimestampForOffset: Sending fetch request for topic: "+topic+", partition: "+partition+", offset: "+offset)
...
var k = new KafkaHelper("dev.unifina:2181")
k.on('ready', function() {
	// k.getTimestampForOffset(k.client, k.consumer, "srqi7IW4Tvq7cyOvXWdn5A", 0, 47480, function(ts, err) {
	// if (err) {
	// 	console.log("Error: "+JSON.stringify(err))
	// }
	// else console.log("Timestamp: "+ts)
	...
	k.getFirstOffsetAfter("srqi7IW4Tvq7cyOvXWdn5A", 0, Date.now(), function(offset, err) {
		if (err) {
			console.log("Error: "+JSON.stringify(err))
		}
