...
KafkaHelper.prototype.getFirstOffsetAfter = function(topic, partition, date, cb) {
	...
	_this.getOffset(topic, true, function(firstOffset) {
		_this.getOffset(topic, false, function(lastOffset) {
			...
			_this.getTimestampForOffset(topic, partition, firstOffset, function(firstDate, firstDateError) {
				if (firstDateError) {
					cb(undefined, firstDateError)
				}
				else _this.getTimestampForOffset(topic, partition, lastOffset-1, function(lastDate, lastDateError) {
					if (lastDateError) {
					    ...
					    else _this.binarySearchOffsetForDate(topic, partition, date, firstOffset, lastOffset-1, function(offset, searchError) {
					    	if (searchError)
...
KafkaHelper.prototype.getTimestampForOffset = function(topic, partition, offset, cb) {
	var _this = this
	...
	setTimeout(function() {
		_this.client.sendFetchRequest(consumer, 
			[{
				topic: topic,
				partition: partition,
				offset: offset,
...
KafkaHelper.prototype.binarySearchOffsetForDate = function(topic, partition, date, low, high, cb) {
	var _this = this
	if (low <= high) {
		// invariants: value > A[i] for all i < low
		//             value < A[i] for all i > high
		var mid = Math.floor((low + high) / 2);
		_this.getTimestampForOffset(topic, partition, mid, function(midDate, err) {
			if (err) {
				cb(undefined, err)
			}
				...
				setTimeout(function() {
					_this.binarySearchOffsetForDate(topic, partition, date, low, high, cb)
				}, 0)
			}
			else if (midDate < date) {
				low = mid + 1;
