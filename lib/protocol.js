...
exports.decode = function(buf, offset, previousOffset) {
	...
	if (version===28) {
		...
		if (contentType === 27) {
			...
			return [version, streamId, ts, ttl, offset, previousOffset, contentType, content]
		}
		else {
...
exports.encode = function(version, timestamp, ttl, streamId, contentType, content) {
	if (version === 28 && contentType === CONTENT_TYPE_JSON) {
		...
		return new BufferMaker()
			...
			.string(streamId)
			// 14 + streamIdLength: contentType (1)
			.Int8(contentType)
			// 15 + streamIdLength: contentLength (4)
			.Int32BE(contentBuf.length)
			// 19 + streamIdLength: content (variable length)
...
var versionFields = {
	'28': ['version', 'streamId', 'timestamp', 'ttl', 'offset', 'previousOffset', 'contentType', 'content']
}
