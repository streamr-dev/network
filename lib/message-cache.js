...
MessageCache.prototype.add = function(msg) {
	...
	if (this.maxSize > 0) {
		// Ensure no gaps
		var lastMsgCounter = msg[constants.COUNTER_KEY]
		var expectedCounter = lastMsgCounter - 1
		var actualCounter = this.messages.length > 0 ? this.messages[this.messages.length-1][constants.COUNTER_KEY] : -1
		...
		if (this.messages.length === 0 || actualCounter === expectedCounter) {
			...
			debug("Oh, no! Cache detected a gap in counters for stream %s! Latest in cache: %d, new message: %d.",
			...
			this.resender.resend(
				...
				function(resentMessage) {     // handler
...
MessageCache.prototype.getRange = function(from, to) {
	...
	if (this.messages.length >= count && this.messages[this.messages.length-1][constants.COUNTER_KEY] >= to && this.messages[0][constants.COUNTER_KEY] <= from) {
