...
CassandraHelper.prototype.query = function(query, queryParams, lastKnownOffset, msgHandler, onDone, onMsgEnd) {
	const _this = this
	this._doQuery(query, queryParams, msgHandler, function(lastOffset) {
		...
		const statusString = "(" + lastOffset + "->" + lastKnownOffset + ")"

		if (lastKnownOffset == null || lastKnownOffset <= lastOffset) {
			debug("Cassandra is up to date on stream" + streamId + ", partition " + partition + " "+ statusString)
			onDone(lastOffset)
		} else {
			debug("Waiting for Cassandra to catch up on stream " + streamId + ", partition " + partition + " behind " +
				(lastKnownOffset - lastOffset) + " messages " + statusString)
			const f = _this.getOffsetRange.bind(_this, streamId, partition, lastOffset + 1, lastKnownOffset, msgHandler,
				onDone, lastKnownOffset)
			setTimeout(f, RETRY_IN_MS)
		}
	})
}

CassandraHelper.prototype._doQuery = function(query, queryParams, msgHandler, onEnd, onError) {
	var lastOffset = null
	this.client.stream(
		query,
		queryParams,
		...
		onEnd(lastOffset) //stream ended, there aren't any more rows
