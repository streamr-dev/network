...
CassandraHelper.prototype.query = function(query, queryParams, msgHandler, doneCallback) {
	this.client.stream(
		...
		while (row = this.read()) {
			...
			var msg = protocol.decode(row.payload, row.kafka_offset.toNumber(), row.previous_offset != null ? row.previous_offset.toNumber() : undefined)
			msgHandler(msg)
		}
	})
	.on('end', function() {
