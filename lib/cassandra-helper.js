...
CassandraHelper.prototype.query = function(query, queryParams, msgHandler, doneCallback) {
	this.client.stream(
		...
		var row;
		while (row = this.read()) {
			var msg = decoder.decode(row.payload, row.kafka_offset)
			msgHandler(msg)
		}
	})
	.on('end', function() {
...
CassandraHelper.prototype.getLast = function(stream, count, msgHandler, doneCallback) {
	this.query(
		"SELECT kafka_offset, payload FROM stream_events WHERE stream = ? ORDER BY kafka_offset DESC LIMIT ?",
		[stream, count],
		msgHandler,
		doneCallback
	)
...
CassandraHelper.prototype.getAll = function(stream, msgHandler, doneCallback) {
	this.query(
		"SELECT kafka_offset, payload FROM stream_events WHERE stream = ? ORDER BY kafka_offset ASC",
		[stream],
		msgHandler,
		doneCallback
	)
...
CassandraHelper.prototype.getFromOffset = function(stream, minOffset, msgHandler, doneCallback) {
	this.query(
		"SELECT kafka_offset, payload FROM stream_events WHERE stream = ? AND kafka_offset >= ? ORDER BY kafka_offset ASC",
		[stream, minOffset],
		msgHandler,
		doneCallback
	)
...
CassandraHelper.prototype.getOffsetRange = function(stream, minOffset, maxOffset, msgHandler, doneCallback) {
	this.query(
		"SELECT kafka_offset, payload FROM stream_events WHERE stream = ? AND kafka_offset >= ? and kafka_offset <= ? ORDER BY kafka_offset ASC",
