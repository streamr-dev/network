...
CassandraHelper.prototype.getLastOffset = function(stream, streamPartition) {
	return this.client.execute("SELECT kafka_offset FROM stream_events WHERE stream = ? AND stream_partition = ? ORDER BY kafka_offset DESC LIMIT 1 ",
		[stream, streamPartition], { prepare: true }).then(function(result) {
			return new Promise(function(resolve, reject) {
				var offset = result.rows[0].kafka_offset
				if (offset !== undefined) {
					resolve(offset)
				} else {
					reject("Cassandra row result doesn't have 'kafka_offset' field.")
				}
			})
		})
}
