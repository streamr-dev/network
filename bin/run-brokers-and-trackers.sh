...
run_application() {
    ...
    # - abbreviate log level so that are levels are just one character (e.g. "error" is longer string than "info" 
    #   and therefore log lines wouldn't align without this transformation)
    (cd ../packages/$2 && STREAMR_APPLICATION_ID=$3 LOG_COLORS=false npm exec $1 -- $5 $6 $7) | awk -v default_color=$4 ' BEGIN { colcode = default_color }
        /^ERROR/ { colcode = "255;0;0" }
        /^WARN/ { colcode = "255;255;0" }
        { print "\033[38;2;" colcode "m" substr($0, 1, 1) substr($0, index($0, " "), length($0)) "\033[39m" ; colcode = default_color }'
}
