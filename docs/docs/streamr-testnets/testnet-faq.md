...
- To participate in the testnets, use specific versions/tags of the Streamr node software, such as `v100.0.0-testnet-three.2`. The `latest` tag still points to the previous milestone (Brubeck) software.
...
- **Node version:** The `pretestnet` tagged releases shouldn't be used anymore, instead use `v100.0.0-testnet-three.2`. 
...
This `v100.0.0-testnet-three.2` version can be used to connect to Testnet 1 **or** the Mumbai testing environment. 
...
```JSON
INFO [2023-11-10T10:52:30.450] (broker              ): Start broker version v100.0.0-testnet-three.2
